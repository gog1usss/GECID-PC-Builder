...
async def get_all_routers(cpu_id: int = None, cur = Depends(get_db_cursor)):
    ...
    if cpu_id:
        query = '''
        select DISTINCT co.id, co.brand_cooler, co.name_cooler, co.max_tdp, co.price, co.image_url, cs.cooler_socket, c.name_cpu, c.tdp 
        from cooler co 
        ...
        await cur.execute('''
        select co.id, co.brand_cooler, co.name_cooler, co.max_tdp, co.price, co.image_url, cs.cooler_socket 
        from cooler co 
        Join cooler_socket cs ON (co.id = cs.cooler_id);
        ''')
